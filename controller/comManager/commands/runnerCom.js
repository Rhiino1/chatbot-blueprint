...
function update(horario, reply) {
  ...
  try {
    for (let index = 0; index < horario.data.columns.length; index++) {
      if (horario.data.columns[index] === 'hiddenRunner') {
    ...
    let runnerArray = runner.split(', ')
    for (let i = 0; i < runnerArray.length; i++) {
      if(!runnerArray[i].startsWith('$')){
        runnerArray[i] = 'twitch.tv/' + runnerArray[i].trim();
      }else{
        runnerArray[i] =  runnerArray[i].substring(1, runnerArray[i].length).trim();
      }
    }

    let prueba = runnerArray.join(' , ')
    if (nw === false) {
      return reply(`${text} ${decodeURIComponent(prueba)}`);
    } else {

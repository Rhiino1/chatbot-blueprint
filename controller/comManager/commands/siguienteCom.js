...
const nextEvent = require('../../../layout/nextEvent')
    ...
    horarioRQ.horario.get(scheduleUrl).then(async function (response) {
      ...
      switch (params['comSec']) {
        ...
        case '-m': {
          ...
          if(socket){
            socket.emit('manual-next', horarioRQ.horaroCounter)
          }
          break;
        }
        case '-mn': {
        ...
        default: {
          ...
          horarioRQ.estimadoEdited = false;
          update(horario, reply);
          // nextEvent.emit('siguiente')
          // await nextRuns();
          if(socket){
            socket.emit('default-next')
          }
          break;
        }
      }
